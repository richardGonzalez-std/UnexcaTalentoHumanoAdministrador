...
import type { Modulo } from "@/app/dashboard/types/dashboard";
  ...
  return (
    <section>
      ...
      <div className="grid grid-cols-2 gap-[14px] max-[520px]:grid-cols-1">
        {modulos.map((modulo) =>
          // Los módulos ya construidos navegan con <Link> (prefetch + transición
          // de cliente); los pendientes siguen con un <a> a "#" hasta que existan.
          modulo.proximamente ? (
            <a key={modulo.titulo} href={modulo.href} className={TARJETA}>
              <Tarjeta modulo={modulo} />
            </a>
          ) : (
            <Link key={modulo.titulo} href={modulo.href} className={TARJETA}>
              <Tarjeta modulo={modulo} />
            </Link>
          )
        )}
...
function Tarjeta({ modulo }: { modulo: Modulo }) {
  return (
    <>
      ...
      <h3 className="mb-[6px] text-[15px] font-bold tracking-[-0.2px] text-ink">{modulo.titulo}</h3>
      <p className="flex-1 text-pretty text-[13px] leading-[1.48] text-ink-soft">
        {modulo.descripcion}
      </p>
      <span
        className={`mt-[14px] inline-flex items-center gap-[6px] text-[13px] font-semibold text-brand ${
          modulo.proximamente ? "opacity-[.55]" : ""
        }`}
      >
        Abrir módulo
        <Icon
